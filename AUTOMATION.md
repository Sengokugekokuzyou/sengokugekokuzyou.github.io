...
- itch.io: GitHub Actionsでゲームをbutlerアップロードし、HP/Discord用の更新情報と開発ログ下書きを作る。
...
`Auto deploy itch.io on game change` が毎時1回、`Sengokugekokuzyou/sengoku-blood-` の最新コミットを確認します。

前回アップロード済みのコミットと違う場合だけ、以下を実行します。

1. ゲーム本体を取得する。
2. releaseフォルダを作る。
3. butlerで `sengokugekokujobeats/kekkanokeifu:html5` へアップロードする。
4. `news.json` に更新情報を追加する。
5. `devlogs/` にitch.io開発ログ用の下書きを作る。
6. `.deploy-state/sengoku-blood-.sha` を最新コミットに更新する。
7. `news.json` 更新により、HPとDiscordへ告知が流れる。

現在のコミットは反映済みとして記録しているため、次に `sengoku-blood-` が更新された時から自動アップロードされます。
...
完全自動アップロードは `sengoku-blood-` のGitHub更新を検知した場合だけ動きます。
