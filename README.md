...
itch.ioページは、配布ページではなく「遊ぶ直前の説得ページ」として整備します。日本語本文、英語要約、スクリーンショット、HP/Discord/YouTube/note導線をセットで更新します。

詳細は `docs/itch-page-policy.md` を参照します。血華の系譜の改善稿は `更新出力/最新フォルダ/itch_2026-06-06_血華の系譜_ページ改善パック.md` に保存します。

## 自動化方針

基本業務はできるだけ自動化します。ただし、公開してよい情報と代表者専用の内部情報は分けて扱います。

優先順位と実装方針は `docs/automation-roadmap.md` を参照します。
...
数字の入力手順は `docs/weekly-summary-input-guide.md` にまとめます。GA4、Search Console、AdSense、itch.io、YouTube Studio、音楽配信、Discordの順に確認します。

`generate-weekly-marketing-summary.yml` は毎週土曜23:00頃に、公開HPの状態、GA4/AdSenseタグ有無、ads.txt、sitemap、`news.json` のYouTube/Apple Music/DistroKid更新を自動取得し、`更新出力/マーケティング/週次マーケティングサマリー_auto_YYYY-MM-DD.md` を作成します。GA4の実数、Search Console、AdSense収益、itch.io閲覧/DL、YouTube Studio詳細はログインが必要なため手入力します。
...
- `DISCORD_WEBHOOK_URL`
- `MARKETING_SUMMARY_DISCORD_WEBHOOK_URL`: 週次マーケティングサマリー専用。公開更新情報チャンネルではなく、代表者だけが見られるDiscordチャンネルのWebhookを設定します。
- `BUTLER_API_KEY`
- `DISTROKID_IMAP_HOST`
- `DISTROKID_IMAP_PORT`
